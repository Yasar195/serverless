...
router.put('/', async (req, res)=> {
    const customer = req.body
    const upload = new Promise((resolve, reject)=> {
        if(customer.customer_id&&customer.customer_name && customer.customer_phone && customer.customer_vehicle && customer.customer_progress && customer.customer_source && customer.dep_id && customer.branch_id && String(customer.customer_pax) && customer.customer_category){
            connection.query(`update customers set customer_name='${customer.customer_name}', customer_phone='${customer.customer_phone}', customer_vehicle='${customer.customer_vehicle}' ${customer.tour_code? `,tour_code='${customer.tour_code}'`: ''}, customer_progress='${customer.customer_progress}', customer_source='${customer.customer_source}', customer_address='${customer.customer_address}', customer_city='${customer.customer_city}', customer_remarks='${customer.customer_remarks}', customer_pax=${customer.customer_pax}, customer_category='${customer.customer_category}', booked=false, tour='${customer.tour}' where customer_id=${customer.customer_id};`, (err)=> {
                if(err){
                    reject()
                }
                else{
